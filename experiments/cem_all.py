...
from QHyper.optimizers.cem_all import AllCEM
...
def compute(angles, weights, samples):
    ...
    solver = QAOA(
        ...
        angles=angles,
        weights=weights,
        hyperoptimizer=AllCEM(epochs=5, samples_per_epoch=samples//5, processes=PROCESSES, disable_tqdm=True)
    )
...
if __name__ == '__main__':
    print(f"CEM all {KNAPSACK_PARAMS}")
